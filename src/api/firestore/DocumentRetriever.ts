...
export const getDocumentWithPathAndId = async (
    ...
    return await firestore().collection(path).doc(documentId).get().then((doc) => {
        if (doc.exists) {
 ...
 * @param path The collection to read form
 * @param criteria The set of instructors to filter documents by
 */

export async function getDocumentWithCriteria(
    path: string,
    criteria: WhereCriteria,
) {
    const query = firestore().collection(path).where(
 ...
 * @param path The collection that the documents are returned form.
...
export const getAllDocumentsWithPath = async (path: string) => {
    ...
    return await firestore().collection(path).get().then((snapshot) => {
